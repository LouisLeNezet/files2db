    ...
    try:
                ...
                if type_infos in dic[month][lecturer].keys():
                    if lateralisation is not None:
                        if isinstance(dic[month][lecturer][type_infos], dict):
                            if (
                                lateralisation
                                in dic[month][lecturer][type_infos].keys()
                            ):
                                if (
                                    value
                                    != dic[month][lecturer][type_infos][lateralisation]
                                ):
                                    dic[month][lecturer][type_infos][lateralisation] = (
                                        union(
                                            dic[month][lecturer][type_infos][
                                                lateralisation
                                            ],
                                            value,
                                        )
                                ...
                                dic[month][lecturer][type_infos].update(
                                    set_value(value, lateralisation)
                                )
                        else:
                            dic[month][lecturer][type_infos] = [
                                dic[month][lecturer][type_infos],
                                {lateralisation: value},
                            ]
                    else:
                        dic[month][lecturer][type_infos] = union(
                            dic[month][lecturer][type_infos], value
                        )

                else:
                    dic[month][lecturer].update(
                        {type_infos: set_value(value, lateralisation)}
                    )
            else:
        ...
        if "File" not in dic[month][lecturer].keys():
            ...
            if disjoint(file_id, dic[month][lecturer]["File"]):
                dic[month][lecturer]["File"] = union(
                    dic[month][lecturer]["File"], file_id
                )
        return dic
    except Exception:
        print_exception()
