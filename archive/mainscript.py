...
def get_platform_infos():
    ...
    try:
        ...
        if operating_system == "Darwin":
            ...
            raise Exception(
                f"You are running on an unsuported system {operating_system}"
            )
    except Exception():
        print_exception()
...
def get_id_colums(field_orga):
    ...
    """
    settings.PARAMS["ID_FIELDS"] = list(
        field_orga.loc[field_orga["Category"] == "Identity", "Field"].values
    )
    settings.PARAMS["ID_SUPL_FIELDS"] = list(
        field_orga.loc[field_orga["Category"] == "IdentitySupl", "Field"].values
    )
...
def main_loop():
    ...
    try:
        ...
        while task_to_do != "Exit":
            ...
            try:
                if (
                    int(os.path.isfile(settings.PARAMS["PATH_TO_ORGA"])) == 1
                    and task_to_do != "Exit"
                ):
                    print_d(
                        "The file used for the repertory of the database will be:\n",
                        settings.PARAMS["PATH_TO_ORGA"],
                    )
                    rep_file = pd.read_excel(
                        settings.PARAMS["PATH_TO_ORGA"], sheet_name="Rep"
                    )
                    rep_file = rep_file.sort_values(
                        by=["ToAdd"], ascending=False, ignore_index=True
                    )
                    rep_file["DateReceptionFichier"] = rep_file[
                        "DateReceptionFichier"
                    ].astype(str)
                    tot_to_add = sum(
                        rep_file.loc[rep_file["ToAdd"] == 1, "LineEnd"]
                        - rep_file.loc[rep_file["ToAdd"] == 1, "LineStart"]
                    )
                    tot_to_check = sum(
                        rep_file.loc[rep_file["ToCheck"] == 1, "LineEnd"]
                        - rep_file.loc[rep_file["ToCheck"] == 1, "LineStart"]
                    )
                    settings.PARAMS["GUI_MW"].count_orga["ToAdd"]["ValueMax"].set(
                        tot_to_add
                    )
                    settings.PARAMS["GUI_MW"].count_orga["ToCheck"]["ValueMax"].set(
                        tot_to_check
                    )
                    field_orga = pd.read_excel(
                        settings.PARAMS["PATH_TO_ORGA"], sheet_name="Field"
                    )
                    values_corres = pd.read_excel(
                        settings.PARAMS["PATH_TO_ORGA"], sheet_name="Values"
                    )
                    formats_corres = pd.read_excel(
                        settings.PARAMS["PATH_TO_ORGA"], sheet_name="Formats"
                    )
                    db_orga = {
                    ...
                    if task_to_do in ["AddDatas", "PrepId", "CompareDB", "CheckCols"]:
                        if task_to_do == "CompareDB":
                            rep_file = rep_file.loc[
                                np.logical_and(
                                    rep_file["ToCheck"] == 1, rep_file["ToAdd"] == 0
                                ),
                            ]
                        else:
                            rep_file = rep_file.loc[
                                np.logical_and(
                                    rep_file["ToCheck"] == 1, rep_file["ToAdd"] == 1
                                ),
                            ]
                        for index in rep_file.index:
                            file_to_skip, file_data, file_id, file_infos = file_to_use(
                                rep_file.loc[index].copy(),
                                task_to_do,
                                database,
                                db_orga,
                            )
                            print(file_infos["FileLoc"])
                            settings.PARAMS["GUI_MW"].update_pb(
                                index + 1,
                                max(rep_file.index) + 1,
                                "files",
                                file_infos["FileLoc"],
                            )
                            if file_to_skip:
                            ...
                            if task_to_do == "CheckCols":
                                ...
                                database.update_doc(
                                    "Files", {"_id": file_id}, {"CheckColsDone": True}
                                )
                                ...
                                for index_dog, dog_file_data in file_data.iterrows():
                                    settings.PARAMS["GUI_MW"].update_pb(
                                        index_dog + 1, max(file_data.index) + 1, "datas"
                                    )
                                    ...
                                    settings.PARAMS["GUI_MW"].update_count_label(
                                        result, task_to_do
                                    )
                                if task_to_do == "PrepId":
                                    database.update_doc(
                                        "Files", {"_id": file_id}, {"PrepIdDone": True}
                                    )
                                    ...
                                    raise Exception(
                                        "Fatal Error, shouldn't access here"
                                    )
                    ...
                    print_d(
                        str(
                            "Couldn't access organisation file:\n"
                            + settings.PARAMS["PATH_TO_ORGA"]
...
def dog_register(
    ...
    try:
        ...
        dog_file_id_col = list(
            set(settings.PARAMS["ID_FIELDS"]).intersection(set(dog_file_data.index))
        )
        dog_file_id_supl_col = list(
            set(settings.PARAMS["ID_SUPL_FIELDS"]).intersection(
                set(dog_file_data.index)
            )
        )

        errors, dog_id, modifs_id, check_later = database.get_dog_id(
            dog_file_data,
            dog_file_id_col,
        ...
        data_to_add_id = dog_file_data[dog_file_id_col + dog_file_id_supl_col].to_dict()
        ...
        if len(errors) == 0 and dog_id is not None:
            if check_later and task_to_do != "CheckErrors":  # DatasToCheck
                ...
                if dog_id == "NeedNew" and new_dog_toadd:  # Need to create new dog
                    ...
                    print_d(
                        str(
                            "Error, dog id given not in the right format "
                            + str(dog_id)
                            + " "
                            + str(type(dog_id))
                        )
                ...
                if modifs_id != []:
                    database.new_modif(file_loc, line, dog_id, dog_file_name, modifs_id)
                    result.append("new_Modif")
                if task_to_do == "AddDatas" and new_dog_toadd:  # Add
                    database.insert_dysplasia_data(
                        dog_id, dog_file_data, file_id, lecteur
                    )
...
def file_to_use(file_infos, task_to_do, database, db_orga):
    ...
    try:
        ...
        db_file_count = database.count("Files", {"NomFichier": file_name})
        if db_file_count == 1:
            db_file = database.find("Files", {"NomFichier": file_name}, "one")
            file_id = db_file["_id"]
            ...
            file_insert = database.update_doc(
                "Files", {"NomFichier": file_name}, file_infos_todb, upsert=True
            )
        ...
        if file_to_skip is False:
            file_data, file_errors = read_file(
                file_path, task_to_do, file_infos, db_orga
            )
            if file_errors != {}:
...
def iterate_data(database, data_name, nb_min_id, ask_user, task_to_do):
    ...
    try:
        ...
        index_doc = 1
        settings.PARAMS["GUI_MW"].update_pb(1, 1, "files", data_name)
            ...
            if result == ["new_Error"]:
                ...
                print_d(
                    str(
                        "A new dog has been "
                        + intersect(
                            result, ["Inserted", "Updated", "new_DatasToCheck"]
                        )[0].lower()
                    )
                )
