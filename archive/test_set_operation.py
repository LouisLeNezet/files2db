...
from archive.set_operation import union, convert_to_set, percent_error
from archive.set_operation import (
    difference,
    joint,
    disjoint,
    intersect,
...
class TestingClass(unittest.TestCase):
    ...
    def test_convert_to_set(self):
        ...
        for value, result, result_m, result_l in zip(
            test_values, test_result, test_result_m, test_result_l
        ):
            ...
            with self.subTest(line=value, alter=True, to_list=True):
                self.assertEqual(
                    set(convert_to_set(value, alter=True, to_list=True)), set(result_l)
                )
    ...
    def test_percent_error(self):
        ...
        test_result = [75, 10]
        test_result_m = [25, 10]
        for value_a, value_b, result, result_m in zip(
            test_values_a, test_values_b, test_result, test_result_m
        ):
        ...
        with self.subTest(error="True"):
            with self.assertRaisesRegex(
                Exception, "Error while calculating the % of difference between"
            ):
    ...
    def test_difference(self):
        ...
        for value_a, value_b, result, result_m in zip(
            ...
            with self.subTest(str_a=value_a, str_b=value_b, alter=False):
                self.assertEqual(
                    set(difference(value_a, value_b, alter=False)), set(result)
                )
            with self.subTest(str_a=value_a, str_b=value_b, alter=True):
                self.assertEqual(
                    set(difference(value_a, value_b, alter=True)), set(result_m)
                )
        with self.subTest(error="True"):
            with self.assertRaisesRegex(
                Exception, "Error while getting the difference between"
            ):
                difference(tuple, "A")
    ...
    def test_joint(self):
        ...
        with self.subTest(error="True"):
            with self.assertRaisesRegex(
                Exception, "Error while getting the joint between"
            ):
                joint(tuple, "A")
    ...
    def test_disjoint(self):
        ...
        with self.subTest(error="True"):
            with self.assertRaisesRegex(
                Exception, "Error while getting the disjoint between"
            ):
                disjoint(tuple, "A")
    ...
    def test_intersect(self):
        ...
        for value_a, value_b, result, result_m in zip(
            ...
            with self.subTest(str_a=value_a, str_b=value_b, alter=False):
                self.assertEqual(
                    set(intersect(value_a, value_b, alter=False)), set(result)
                )
            with self.subTest(str_a=value_a, str_b=value_b, alter=True):
                self.assertEqual(
                    set(intersect(value_a, value_b, alter=True)), set(result_m)
                )
        with self.subTest(error="True"):
            with self.assertRaisesRegex(
                Exception, "Error while getting the intersect between"
            ):
                intersect(tuple, "A")
    ...
    def test_union(self):
        ...
        for value_a, value_b, result, result_m in zip(
            ...
            with self.subTest(str_a=value_a, str_b=value_b, alter=True):
                self.assertEqual(
                    set(union(value_a, value_b, alter=True)), set(result_m)
                )
        with self.subTest(error="True"):
            with self.assertRaisesRegex(
                Exception, "Error while getting the union between"
            ):
                union(tuple, "A")
    ...
    def test_match(self):
        ...
        with self.subTest(error="True"):
            with self.assertRaisesRegex(
                Exception, "Error while getting the match between"
            ):
                match(tuple, "A")
