...
def num_id_sep(string_to_sep):
    ...
    try:
        ...
        logging.info(
            str("Error for " + str(pu_tat_array) + " " + str(type(pu_tat_array)))
        )
        return ["Error", "Error"]
                ...
                if date_to_convert == "00:00:00" or date_to_convert == "0000-00-00":
                    ...
                    if SHORT_DATE_F.match(date_to_convert):
                        if date_to_convert[-2:] > "80":  # Siecle dernier
                            new_date = str(
                                date_to_convert[0:6] + "19" + date_to_convert[-2:]
                            )
                        else:
                            new_date = str(
                                date_to_convert[0:6] + "20" + date_to_convert[-2:]
                            )
...
def norm_data(file):
    ...
    try:
        ...
        nom_cols = [
            col for col in col_present if ("NomUsuel" in col or "NomChien" in col)
        ]
        ...
        if len(nom_cols) > 0:
            ...
            file["AllName"] = file.apply(
                lambda row: get_not_null(row["AllName"]), axis=1
            )
            file["NomChien"] = file.apply(
                lambda row: conca_name(row["AllName"]), axis=1
            )

        cols_date = [x for x in col_present if "Date" in x]
        for col_date in cols_date:
            if file[col_date].dtypes == "datetime64[ns]":
                file[col_date] = pd.to_datetime(file[col_date]).apply(
            ...
            file[col_date] = file[col_date].replace(r"-", ".", regex=True)
            file[col_date] = file[col_date].replace(
                r"(?<=[\d])et(?=\d)", "_", regex=True
            )
            ...
            errors_present = (
                file[str(col_date + "Error")].astype(str).str.contains("WrongFormat")
            )
            if any(errors_present):
                errors.update(
                    {
                        col_date: error_register(
                            file.loc[
                                errors_present,
                                ["NomChien", col_date, str(col_date + "Error")],
                            ]
                        )
                    }
                )
        ...
        if "NumIdentification" in col_present:
            file["NumIdentification"] = file["NumIdentification"].replace(
                r"\.", "", regex=True
            )
            file["NumIdentification"] = file["NumIdentification"].replace(
                r"\s", "", regex=True
        ...
        if "Puce" in list(set(file.columns)):
            ...
            errors_present = (
                file["PuceError"].astype(str).str.contains("WrongFormat|Not an int")
            )
            if any(errors_present):
                errors.update(
                    {
                        "Puce": error_register(
                            file.loc[errors_present, ["NomChien", "Puce", "PuceError"]]
                        )
                    }
                )
                logging.info(
                    str(file.loc[errors_present, ["NomChien", "Puce", "PuceError"]])
                )
        ...
        if "Tatouage" in list(set(file.columns)):
            file["Tatouage"] = (
                file["Tatouage"].astype(str).replace(r"\s", "", regex=True)
            )
            file["Tatouage"] = file["Tatouage"].str.upper()
            file["TatouageError"] = file.apply(
                lambda row: check_tatouage(row["Tatouage"]), axis=1
            )
            errors_present = (
                file["TatouageError"].astype(str).str.contains("WrongFormat")
            )
            if any(errors_present):
                errors.update(
                    {
                        "Tatouage": error_register(
                            file.loc[
                                errors_present,
                                ["NomChien", "Tatouage", "TatouageError"],
                            ]
                        )
                    }
                )
                logging.info(
                    str(
                        file.loc[
                            errors_present, ["NomChien", "Tatouage", "TatouageError"]
                        ]
                    )
                )
        ...
        if "NumLof" in col_present:
            try:
                ...
                if any(num_lof_errors):
                    errors.update(
                        {
                            "NumLof": error_register(
                                file.loc[num_lof_errors, ["NomChien", "NumLof"]]
                            )
                        }
                    )
            ...
            if any(errors_present):
                errors.update(
                    {
                        "NumLof": error_register(
                            file.loc[errors_present, ["NomChien", "NumLof"]]
                        )
                    }
                )
        ...
        for col in dys_cols:
            if col != "Lecteur":
                ...
                file[col] = file[col].str.replace(" ", "")
                file[col] = file[col].str.replace(
                    r"^(NONMESURE|NONEFFECTUE|NONMESURABLE|NM|\?*)$", "None", regex=True
                )

                if (
                    any(file[col].str.contains("_"))
                    and "Infos" not in col
                    and "Parents" not in col
                ):
            ...
            if "ID" in col or "NO" in col or "FCI" in col:
                if "_DG" in col:
                    if any(file[col].str.contains("/")):
                        file[[col.replace("_DG", "_D"), col.replace("_DG", "_G")]] = (
                            file[col].str.split("/", expand=True)
                        )
                        errors_present = np.logical_and(
                            [not_null(x) for x in file[col.replace("_DG", "_G")]],
                            [is_null(x) for x in file[col.replace("_DG", "_D")]],
                        )
                        if any(errors_present):
                            file.loc[errors_present, col.replace("_DG", "_G")] = (
                                file.loc[errors_present, col.replace("_DG", "_D")]
                            )
                            errors.update(
                                {
        ...
        for col in dys_cols:
            ...
            if "ID" in col or "NO" in col:
                ...
                if any(errors_present):
                    errors.update(
                        {
                            str(col + "_ToNumError"): error_register(
                                file.loc[errors_present, ["NomChien", col]]
                            )
                        }
                    )
                ...
                file[col] = file[col].replace(
                    r"(?<=[A-E])\sOU\s(?=[A-E])", "_", regex=True
                )
            elif "Lecteur" in col:
                file[col] = file[col].replace(".", "")
                file[col] = file[col].replace("(^DR )|(^D )", "", regex=True)
                file[col] = file[col].replace(
                    "(/ACGAO)|(, NON OFFICIEL)", "", regex=True
                )
                file[col] = file[col].replace(r"\(.*\)", "", regex=True)
                lecteur_replace = {
                    "YVES LIGNEREUX": "LIGNEREUX",
                    "DIDIER FAU": "FAU",
        ...
        if "NumCaniDNA" in col_present:
            ...
            datas_conv, num_cani_dna_errors = convert_to_num(
                file["NumCaniDNA"], "Int32"
            )
            if any(num_cani_dna_errors):
                errors.update(
                    {
                        "NumCaniDNA": error_register(
                            file.loc[num_cani_dna_errors, ["NumCaniDNA"]]
                        )
                    }
                )
        ...
        if "AffixeChien" in col_present:
            ...
            file["AffixeChien"] = file["AffixeChien"].replace(r"\s*$", "", regex=True)
            file["AffixeChien"] = file["AffixeChien"].replace(
                r"^\/du cesecah$|^cesecah$", "du cesecah", regex=True
            )
            file["AffixeChien"] = file["AffixeChien"].replace(
                r"\/cesecah", "", regex=True
            )
...
def convert_to_num(datas, totype="Int32"):
    try:
        ...
        datas_conv = pd.DataFrame(
            {k: pd.to_numeric(v, errors="coerce") for k, v in all_datas.items()}
        )
        ...
        all_datas_conv = pd.Series(
            [get_not_null(x) for x in datas_conv.to_numpy()], dtype=object
        )
        all_datas_conv = pd.Series(
            [
                np.NaN if is_null(x) else x[0] if len(x) == 1 else x
                for x in all_datas_conv
            ],
            dtype=object,
        )
...
def check_columns(file, field_orga):
    ...
    try:
        ...
        field_names = set(
            field_orga.loc[field_orga["Category"] != "Dysplasia", "Field"]
        )
        dysplasia_field = set(
            field_orga.loc[field_orga["Category"] == "Dysplasia", "Field"]
        )
        ...
        for field in file_field:
            ...
            if len(field_vars) != 1:  # Should be a dysplasia field
                try:
                    ...
                    if (
                        field_vars[1] == "NA"
                        or field_vars[1] == "PostOpe"
                        or field_vars[1] == "Lecteur"
                    ):
                        check2 = True
                    else:
                ...
                if field in field_names or field == "NA":
                    ...
                    logging.info(
                        "Field not separable and not present in possible fields"
                    )
