...
def data_sep_del(data_df, params, sep="_&@_"):
    """
    Separate data, and delete all/partial/in values from data.

    Parameters
    ----------
    data_df : DataFrame
        DataFrame with the value to normalize.
    params : Dict
        Dictionnary with all the parameters to use.
        The recognize parameters are "Sep", "Del", "StripFrom", "DelIn" and
        will be used in this order.
    sep : string, optional
        String used to separate the values in the data.
        Default is "_&@_"
    ...
    -------
    data_df : DataFrame
        DataFrame with all values normalized, dimension can be greater than original.

    """
    try:
        # Iterate through all the available columns in data frame
        for col in data_df:
            data_se = data_df[col]
            # First separate the values based on the Sep property
            if "Sep" in params.keys() and not_null(params["Sep"], True):
                for mod_sep in str(params["Sep"]).split(","):
                    data_se = data_se.str.replace(mod_sep, sep, regex=True)
                data_se = data_se.str.split(sep, expand=True)
                data_se.fillna("", inplace=True)

            #  Change Series to dataframe
            if isinstance(data_se, pd.Series):
                col = data_se.name
                data_se = data_se.to_frame()
                if data_se.columns == col:
                    data_se.rename(columns={col: 0}, inplace=True)

            # Register each new column with an _ separator and a number
            for numcol in data_se:
                data_df.loc[:, col+"_"+str(numcol)] = data_se[numcol]
        ...
        for col in data_df:
            data_se = data_df[col]
            #  Delete value if complete match
            if "Del" in params.keys() and not_null(params["Del"], True):
                for mod_del in params["Del"].split(","):
                    data_se = data_se.replace(mod_del, "")

            #  Delete all characters situated after
            if "StripFrom" in params.keys() and not_null(params["StripFrom"], True):
                for mod_strip in params["StripFrom"].split(","):
                    data_se = data_se.str.partition(mod_strip)[0]

            #  Delete value inside string
            if "DelIn" in params.keys() and not_null(params["DelIn"], True):
                for mod_del in params["DelIn"].split(","):
                    data_se = data_se.str.replace(mod_del, "", regex=True)
            data_df[col] = data_se

        return data_df
    except Exception as exc:
        print_exception()
        raise RuntimeError("Error while preformatting data") from exc

def data_sep_pat(data_df, df, col_for_rename, params):
    """
    ...
    ----------
    data_df : DataFrame
        DataFrame from which values should be separated.
    df : DataFrame
        Original dataframe for the columns that should be externalized.
    col_for_rename : str
        Names of the column to use for the renaming process.
    params : Dict
        All parameters to use.
    ...
    try:
        ...
        if "ToSep" in params.keys() and params["ToSep"] == "T":
            sep_names_nb = [re.match(r"Sep(\d)Name", param)[1]
                            for param in params.keys() if re.match(r"Sep(\d)Name", param)]
            sep_names = [params["Sep"+num+"Name"] for num in sep_names_nb]
            sep_pats_nb = [re.match(r"Sep(\d)Pat", param)[1]
                            for param in params.keys() if re.match(r"Sep(\d)Pat", param)]
            if sep_names_nb == sep_pats_nb and len(sep_names_nb) > 1:
                for col in data_df:
                    #  Separate data based on patterns available
                    for num in sep_names_nb:
                        #  Check if all patterns are of the forms (?P<Data>)(?P<Other>)
                        pat_simple = ''.join(re.findall(r"\(|\)|\?P<Other>|\?P<Data>",
                                                        params["Sep"+num+"Pat"]))
                        pat_match = re.findall(r"\(\)", pat_simple)
                        if len(pat_match) > 0:
                            raise ValueError("Groups should be named Data or Others in {params['Sep'+num+'Pat']}")

                        sep_name = str(col)+"_"+params["Sep"+num+"Name"]
                        data_match = data_df[col].str.extractall(params["Sep"+num+"Pat"], flags=re.IGNORECASE)
                        full_match = data_df[col].str.fullmatch(params["Sep"+num+"Pat"], flags=re.IGNORECASE)

                        full_match = list(full_match[full_match].index)
                        match_col = [x not in ["Other", "Data"] for x in data_match.columns]
                        if any(match_col):
                            raise TypeError("Error another group generate with match different to Data or Other")

                        data_to_use = [data_match["Data"].loc[(ind, 0)]
                                        if ind in full_match
                                        else None
                                        for ind in data_df.index]

                        data_df.loc[:, sep_name] = data_to_use

                    #  Check for all errors generated
                    errors_df = pd.DataFrame(index=data_df.index)
                    errors_df["DataLeft"] = data_df[col]
                    for num in sep_names_nb:
                        sep_name = str(col)+"_"+params["Sep"+num+"Name"]
                        errors_df[sep_name+"_Error_Match"] = [(f"Pattern {params['Sep'+num+'Name']} match same part as previous pattern"
                                                                if b not in a else None)
                                                            if not_null(b)
                                                            else None
                                                            for a, b in zip(errors_df["DataLeft"],
                                                                            data_df[str(col)+"_"+params["Sep"+num+"Name"]]) ]
                        errors_df["DataLeft"] = [a.replace(b, "", 1) if not_null(a) and not_null(b) else a
                                                for a, b in zip(errors_df["DataLeft"],
                                                                data_df[str(col)+"_"+params["Sep"+num+"Name"]])]

                    errors_df["Error_None"] = [f"'{dataleft}' is left after applying all paterns"
                                                if not_null(re.findall("[A-Z0-9]", dataleft, flags=re.IGNORECASE))
                                                else None
                                                for dataleft in errors_df["DataLeft"]]

                    col_matchboth = [col for col in errors_df.columns if "_Error_Match" in col]
                    errors_df["Error_Match"] = [get_not_null(err_ind)
                                                for err_ind in errors_df[col_matchboth].values.tolist()]

                    errors.append([{"When": "SeparationOnPattern",
                                    "Which": data_df.loc[ind, col],
                                    "Error": get_not_null(errors_df.loc[ind, ["Error_None", "Error_Match"]].tolist())}
                                    if not_null(get_not_null(errors_df.loc[ind, ["Error_None", "Error_Match"]].tolist()))
                                    else {} for ind in data_df.index])

                    #  Delete original column from the data
                    del data_df[col]

                #  If the SepKeepLink is false, then the data should be added as separated columns
                if "SepKeepLink" in params.keys() and params["SepKeepLink"] == "F":
                    logging.info(data_df)
                    for num in sep_names_nb:
                        df.loc[:, params["Sep"+num+"Name"]] = conca_simplify(data_df.loc[:, [f"{col}_{params['Sep'+num+'Name']}" for col in columns]])
                    col_for_rename = None
                else:
                    for col in columns:
                        col_for_rename.update({col: sep_names})

            else:
                raise ValueError("Error all Pattern and Name not given")
        if not_null(errors):
            all_errors = [value for value in np.column_stack(errors).tolist()]
        return data_df, df, all_errors, col_for_rename
    except Exception as exc:
        print_exception()
        raise RuntimeError("Error while separating data based on pattern") from exc
...
def data_replace(data_se, params, to_lower=True):
    """
    Replace all values in a serie based on correspondancies.
    ...
    data_se : Serie
        Serie to normalize.
    params : Dict
        Dictionnary listing all pairs to convert from.
    to_lower : Boolean, optional
        Should all values be converted to lowercase prior to conversion. The default is True.
    ...
    -------
    data_se : Serie
        Same serie but with all values normalize based on correspondancies.

    """
    try:
        replace_dict = {}
        for corres in params:
            all_keys = corres["Eq"].split(",")
            if to_lower:
                all_keys = [v.lower() for v in all_keys]
                data_se = data_se.str.lower()
            replace_dict.update({key: corres["Value"] for key in all_keys})
        data_se = data_se.replace(replace_dict)
        return data_se
    except Exception as exc:
        print_exception()
        raise RuntimeError("Error while replacing values by correspondancies") from exc
