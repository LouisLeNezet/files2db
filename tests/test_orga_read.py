...
class TestingLoadFileOrga(unittest.TestCase):
    ...
    def test_load_file_orga(self):
        ...
        self.assertEqual(
            ...
            [
                'FilePath', 'SheetName', 'LineStart', 'LineEnd',
                'Header', 'ColStart', 'ColEnd', 'ToAdd', 'AsCorrection'
            ]
        )
...
class TestColumnValidation(unittest.TestCase):
    ...
    def test_validate_columns_missing_columns(self):
        ...
        with self.assertRaises(KeyError) as context:
            validate_columns(
                pd.DataFrame(columns=["A"]), "test.xlsx",
                cols_need={"A", "B"}, cols_sup=False
            )
    ...
    def test_validate_columns_extra_columns(self, mock_log):
        ...
        validate_columns(
            pd.DataFrame(columns=["A", "B", "C"]), "test.xlsx",
            cols_need={"A", "B"}, cols_sup=False
        )
        mock_log.assert_called_once_with(
