...
class TestValidateFiles(unittest.TestCase):
    ...
    def test_iterate_with_file_csv(self):
        ...
        columns_expected = [
            "EmplacementOrigine", "ColA", "ColB", "ColC", "ColD", "Author", "DateFileCreation", "FileName"
        ]
        for col in columns_expected:
            self.assertIn(col, iterated_data.columns, f"Column {col} should be present in the DataFrame.")
        ...
        self.assertEqual(
            iterated_data.shape[0], 7, "The DataFrame should have 7 rows."
        )
        self.assertEqual(
            iterated_data.shape[1], 10, "The DataFrame should have 10 columns."
        )
